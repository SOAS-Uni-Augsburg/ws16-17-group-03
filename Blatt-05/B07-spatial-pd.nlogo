...
to setup
  clear-all
  set nearby moore-offsets 1 false
  ...
  ask patches [
    set acted? false
    set last-partner-action -1
    set got-betrayed? false
    set score 0
    set scores array:from-list n-values 8 [0]
    set strategy one-of possible-strategies
    if Kampf?
    [
      ifelse random 2 = 1
      [set strategy Method1]
      [set strategy Method2]
    ]
    update-color
  ]
...
to interact  ;; patch procedure
  ...
  let otherpatch one-of patches at-points nearby with [acted? = false]
  if otherpatch != nobody
  [
  set-action
  ask otherpatch [set-action]

  get-payoff otherpatch
  ask otherpatch[ get-payoff myself]
  ]
end
...
to select-strategy  ;; patch procedure
  ...
  let best-patch one-of patches at-points nearby with-max [score]
  let best-strategy [strategy] of best-patch
...
to get-payoff [partner]

  let partner-action [action] of partner
  let temp-payoff 0
  ifelse action = 0
    [
      ifelse partner-action = 0
      [set temp-payoff payoff-r]
      [set temp-payoff payoff-s
        set got-betrayed? true]
    ]
    [
      ifelse partner-action = 0
      [set temp-payoff payoff-t]
      [set temp-payoff payoff-p
        set got-betrayed? true]
    ]
     set last-partner-action partner-action
    array:set scores (ticks mod 8) temp-payoff
    set score sum array:to-list scores
end
...
true
false
"" "set-plot-y-range 0 32"
PENS
"default" 1.0 1 -16777216 true "" "histogram [0 - score] of patches"
...
367
Best Score
[score] of one-of patches with-max [score]
17
1
11
...
417
Worst Score
[score] of one-of patches with-min [score]
17
1
11
...
Method2
Method2
"random" "always cooperate" "always defect" "grim trigger" "tit for tat"
2

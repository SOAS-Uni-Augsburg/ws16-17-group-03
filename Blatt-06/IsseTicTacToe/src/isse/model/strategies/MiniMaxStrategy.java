...
public class MiniMaxStrategy extends RandomStrategy
	...
	public Move getMove(GameBoard board) {
		...
		int[] result = minimax(SUCHTIEFE, current); // depth, max turn
		_sum += _counter;
		// System.out.println("Current: " + current.name());
		// System.out.println("Sum: " + _sum + "\ncounter: " + _counter);
		// System.out.println("_____________________Max. Tiefe: " + temp);
	...
	private int[] minimax(int depth, Player player) {
		...
		int bestScore = (player == current) ? Integer.MIN_VALUE
				: Integer.MAX_VALUE;
		int currentScore;
		int bestRow = -1;
		int bestCol = -1;
		...
		if (nextMoves.isEmpty() || depth == 0) {
			...
			temp = Math.max(temp, (SUCHTIEFE - depth));
			bestScore = evaluate();
		} else {
			for (int[] move : nextMoves) {
				...
				if (player == current) { // mySeed (computer) is maximizing
					...
					currentScore = minimax(depth - 1, current)[0];
					if (currentScore < bestScore) {
						bestScore = currentScore;
						bestRow = move[0];
						bestCol = move[1];
